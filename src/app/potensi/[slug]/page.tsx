...
const getSettingsData = async () => {
  try {
    return await readData<any>('settings.json');
  } catch (e) {
    return {};
  }
}
  ...
  return (
    <div className="flex flex-col flex-grow w-full bg-surface-bright min-h-screen pt-28 md:pt-32 pb-32">
      <article className="max-w-6xl w-full mx-auto px-margin-mobile md:px-margin-desktop">
        ...
        <Link href="/potensi" className="inline-flex items-center gap-2 text-primary hover:text-primary/80 transition-colors font-label-md font-semibold mb-8 bg-primary/5 px-4 py-2 rounded">
          <ArrowLeft className="w-5 h-5" />
          Kembali ke Eksplorasi Potensi
        </Link>
        
        <div className="w-full aspect-video md:aspect-[21/9] rounded-md overflow-hidden mb-12 shadow-sm border border-surface-variant/30 relative group bg-surface-container-low">
          {potensi.image ? (
          ...
          <div className="absolute top-6 left-6 bg-white/95 border border-white/30 text-primary px-5 py-2 rounded font-label-sm text-[14px] font-semibold flex items-center gap-2 shadow-sm">
            {getIconForCategory(potensi.category)}
            {potensi.category}
          </div>
          
          <div className={`absolute top-6 right-6 px-5 py-2 rounded font-label-sm text-[14px] font-bold shadow-sm border ${
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 flex flex-col gap-10">
            ...
            {potensi.metrics && potensi.metrics.length > 0 && (
              <div className="bg-white border border-surface-variant/30 rounded-md p-8 md:p-10 shadow-sm">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-6">
                  {potensi.metrics.map((metric: any, idx: number) => (
                    <div key={idx} className="flex flex-col items-center justify-center p-6 text-center bg-surface-container-low rounded-md shadow-inner border border-surface-variant/20">
          ...
          <aside className="lg:col-span-1 flex flex-col gap-8">
            ...
            <div className="bg-white p-8 rounded-md shadow-sm border border-surface-variant/30">
              <h3 className="font-title-lg text-xl text-primary mb-6 flex items-center gap-3 font-semibold">
            ...
            <div className="bg-primary p-8 rounded-md shadow-sm text-white relative overflow-hidden group">
              <div className="absolute -bottom-10 -right-10 w-40 h-40 bg-white/10 rounded-full group-hover:bg-white/20 transition-colors duration-500"></div>
              ...
              <a href={`https://wa.me/${settings.telepon ? settings.telepon.replace(/^0/, '62') : '6281234567890'}`} target="_blank" rel="noreferrer" className="bg-white hover:bg-white/90 text-primary p-4 rounded-md font-label-md flex items-center justify-center gap-3 font-bold break-all relative z-10 shadow-lg transition-colors cursor-pointer w-full text-center">
                <Phone className="w-5 h-5" />
                {potensi.contact || settings.telepon || "Hubungi Admin Desa"}
              </a>
            </div>
